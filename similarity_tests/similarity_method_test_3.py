...
def calculate_similarity(method, query, documents, column_name=None):
    if method == "TF-IDF":
...
def compare_methods(df, query, methods, column_name=None):
    ...
    for method in methods:
        if column_name is not None:
            ...
            documents = df.iloc[:, 0].tolist()  # Use the first column as the text data
        
        scores = calculate_similarity(method, query, documents)
        
        # Ensure that binary_scores and true_labels have the same length
        binary_scores = [1 if score > 0.3 else 0 for score in scores]
        true_labels = [1] * len(scores)  # len(scores) = number of samples
...
# specify the column name with text data (or None for the first column)
column_name = "title"

# Specify a query
query = "apple company"  # Change the query as needed

# Call the compare_methods function with the DataFrame, query, methods, and column_name
result_df = compare_methods(preprocessed_df, query, methods_to_compare, column_name)
